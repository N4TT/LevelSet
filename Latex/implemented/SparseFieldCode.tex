...
\ForAll{$e \in Lz)$}
	...
	\If{$phi(e.x, e.y) \geq 0.5$} \Comment{$phi = \phi$}
		\State sp1.add(e)
		\State Lz.remove(e)
...
\ForAll{$e \in Ln1)$}
	\If{$e$ has no neighbour that is part of Lz}
		\State Sn2.add(e)
		\State Ln1.remove(e)
	\Else
		\State $M =$ Greatest value from $N(e)$ with $N(e) \geq 0$ 
		\Statex \Comment{$N(e) = neighbour $ pixels of e in $label$}
		\State $phi(e.x, e.y) = M-1$
		\If{$phi(e.x, e.y) \geq -0.5$}
			\State Sz.add(e)
			\State Ln1.remove(e)
		\ElsIf{$phi(e.x, e.y) < -1.5$}
			\State Sn2.add(e)
			\State Ln1.remove(e)
		\EndIf
	\EndIf
...
\begin{algorithmic}[1] %[n] means numbering of the n'th lines, so [1] means numbering of each line
\ForAll{$e \in Ln2)$}
	\If{$e$ has no neighbour that is part of Ln1}
	...
	\Else
		...
		\State $phi(e.x, e.y) = M-1$
		\If{$phi(e.x, e.y) \geq -1.5$}
			\State Sn1.add(e)
			\State Ln2.remove(e)
		\ElsIf{$phi(e.x, e.y) < -2.5$}
			\State $label(e.x, e.y) = -3$
			\State $phi(e.x, e.y) = -3$
			\State Ln2.remove(e)
		\EndIf
...
\ForAll{$e \in Lp2)$}
	\If{$e$ has no neighbour that is part of Lp1}
		\State $label(e.x, e.y) = 3$
		\State $phi(e.x, e.y) = 3$
		\State Lp2.remove(e)
	\Else
...
\begin{algorithmic}[1]
	...
	\ForAll{$e \in Sn1)$}
		...
		\State Ln1.add(e)
		\ForAll{$ne \in N(e)$} \Comment{The four neighbouring pixels: over, under, left and right}
			\If{$phi(ne.x, ne.y) == -3$}
				\State Sn2.add(ne)
			\EndIf
		\EndFor
