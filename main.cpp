...
void displayShortVolume(Volume<short> * V, int3 seed, int display){ 
	...
	if(display == 0){ //display input data
		...
		for(int z = 0; z < V->getDepth(); z++) {
			float2 list;
			list.x = ((float)V->get(x,y,z) + 1000.0f)/ 2000.0f;		//even tho the input can have any value allowed by short, its known to be between -1000 and 1000
			int3 n(x,y,z);	
			image[x][y][z] = ((int)V->get(x,y,z) + 1000.0f)/ 2000.0f;
			if(sqrt((float)((seed.x-n.x)*(seed.x-n.x)+(seed.y-n.y)*(seed.y-n.y)+(seed.z-n.z)*(seed.z-n.z))) < 5.0f){
				list.y = 1.0f;
			}
			v2->set(n, list);
